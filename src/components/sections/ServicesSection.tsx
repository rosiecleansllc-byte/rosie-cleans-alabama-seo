...
export default function ServicesSection({ services, cityName }: ServicesSectionProps) {
  ...
  const otherAddOns = addOns.filter((s) =>
    !["inside-oven", "inside-fridge", "inside-cabinets", "range-hood", "stovetop"].includes(s.id)
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-12">
          {primaryServices.map((service) => (
            <a
              ...
              className="group bg-white rounded-2xl p-6 border border-gray-100 hover:border-brand-green/40 hover:shadow-lg transition-all duration-200 flex flex-col"
            >
              <div className="w-12 h-12 bg-brand-green/10 text-brand-green rounded-xl flex items-center justify-center mb-4 group-hover:bg-brand-green group-hover:text-white transition-all duration-200 flex-shrink-0">
                {serviceIcons[service.icon] || serviceIcons.sparkles}
              </div>
              <h3 className="font-bold text-gray-900 text-base mb-2 group-hover:text-brand-green transition-colors">
                {service.name}
              </h3>
              <p className="text-gray-600 text-sm leading-relaxed flex-1">
                {service.description}
              </p>
              <div className="mt-4 flex items-center text-brand-green text-sm font-semibold">
        ...
        {addOns.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-6 sm:p-8">
            ...
            <p className="text-gray-600 text-sm mb-6">
              Enhance any cleaning with these optional add-ons.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {otherAddOns.map((service) => (
                <div key={service.id} className="flex items-start gap-3">
                  <div className="w-9 h-9 bg-brand-green/10 text-brand-green rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                    {serviceIcons[service.icon] || serviceIcons.sparkles}
                  </div>
                  <div>
              ...
              {kitchenAddOns.length > 0 && (
                <div className="sm:col-span-2">
                  <p className="font-semibold text-gray-900 text-sm mb-3">Kitchen Essentials</p>
